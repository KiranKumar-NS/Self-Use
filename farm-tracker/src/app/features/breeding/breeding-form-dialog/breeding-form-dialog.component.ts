...
export class BreedingFormDialogComponent implements OnInit {
  ...
  matingMethod: 'natural' | 'artificial' = 'natural';
  expectedDeliveryDate: Date | null = null;
  gestationDays: number | null = null;
  actualDeliveryDate: Date | null = null;
  offspringCount: number | null = null;
  offspringMale: number | null = null;
  offspringFemale: number | null = null;
  complications = '';
  note = '';
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (this.data?.record) {
      ...
      this.selectedSegment = r.segment;
      this.segmentName = r.segmentName;
      this.status = r.status;
      this.damId = r.damId;
      this.damName = r.damName;
      this.sireId = r.sireId || '';
      this.sireName = r.sireName || '';
      this.matingDate = r.matingDate.toDate();
      this.matingMethod = r.matingMethod || 'natural';
      this.expectedDeliveryDate = r.expectedDeliveryDate?.toDate() || null;
      this.gestationDays = r.gestationDays || null;
      this.actualDeliveryDate = r.actualDeliveryDate?.toDate() || null;
      this.offspringCount = r.offspringCount || null;
      this.offspringMale = r.offspringMale || null;
      this.offspringFemale = r.offspringFemale || null;
      this.complications = r.complications || '';
      this.note = r.note || '';
  ...
  async save(): Promise<void> {
    ...
    try {
      ...
      if (this.expectedDeliveryDate) recordData.expectedDeliveryDate = Timestamp.fromDate(this.expectedDeliveryDate);
      if (this.gestationDays) recordData.gestationDays = this.gestationDays;
      if (this.status === 'delivered') {
        if (this.actualDeliveryDate) recordData.actualDeliveryDate = Timestamp.fromDate(this.actualDeliveryDate);
