              ...
              @for (supplier of paginatedSuppliers(); track supplier.id) {
                <tr>
                  ...
                  <td class="actions-cell">
                    <button mat-icon-button (click)="editSupplier(supplier)" title="Edit" aria-label="Edit supplier"><mat-icon>edit</mat-icon></button>
                    <button mat-icon-button color="warn" (click)="confirmDelete(supplier)" title="Delete" aria-label="Delete supplier"><mat-icon>delete</mat-icon></button>
                  </td>
                </tr>
              }
            </tbody>
...
export class SupplierListComponent implements OnInit {
  ...
  async confirmDelete(supplier: Supplier): Promise<void> {
    const ref = this.dialog.open(ConfirmDialogComponent, {
      data: { title: 'Delete Supplier', message: `Delete "${supplier.name}"?`, confirmText: 'Delete', showDeleteOptions: true } as ConfirmDialogData,
    });
    ref.afterClosed().subscribe(async (result) => {
      if (result?.confirmed) {
        try {
          if (result.deleteType === 'hard') {
            await this.supplierService.hardDelete(supplier.id);
          } else {
